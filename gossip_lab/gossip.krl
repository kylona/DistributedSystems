ruleset gossip {
  meta {
    ...
    shares get_sensorID, gossip_ledger, seenMap, TX_to_SensorID, gossip_to_spread, pickSomeTea, peers, peersSensorID, getSensorIDFromTX, getTXFromSensorID, seenForSensorID, TX_Map, SensorID_Map, schedule, heartbeat_period, gossip_state
  }
  ...
  global {
    ...
    seenMap = function() {
      //Gets the largest number for which we have all messages less then that number
      knownSensorIDs = ent:gossip_to_spread.keys().reduce(function(accumulator, messageID) {
        sensorQ = messageID.split(":")[0]
        return accumulator.append(sensorQ)
      }, [])
    ...
    pre {
      ...
      |
        ({ "eci": eci,
          ...
          "attrs": {
            "messageID": tea.get(["gossip", "messageID"]),
            "sensorID": sensorID,
            "temperature": tea.get(["gossip", "temperature"]),
            "time": tea.get(["gossip", "time"]),
            "sourceID": ent:sensorID,
          }
        })
