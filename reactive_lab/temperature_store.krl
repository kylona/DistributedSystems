ruleset temperature_store {
  meta {
    ...
    description <<
		>>
    author "Kyle Storey"
    provides temperatures, threshold_violations, inrange_temperatures, currentTemp
    shares temperatures, threshold_violations, inrange_temperatures, currentTemp
  }
   
  global {
    currentTemp = function() {
      temp = ent:currentTemp
      temp.klog("CurrentTemp:")
    }
  ...
  rule respond_to_temp_request_event {
    select when sensor gatherTemperatures
    pre {
      mostRecent = currentTemp()
      response = {
        "Tx": event:attrs{"Tx"},
        "correlationID": event:attrs{"correlationID"},
        "temperature": mostRecent,
      }
      respondECI = event:attrs{"Rx"}.klog("RESPOND ECI")
    }
    event:send(
      { "eci": respondECI, 
        "eid": "respond_gather_temp_request", // can be anything, used for correlation
        "domain": "sensor", "type": "temperatureResponse",
        "attrs": response
      }
    )
  }
